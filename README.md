...
### 배경 & 목적
실제 교육 수강을 위해 다니던 학원에서, 잦은 외부 출장과 인력 부족으로 인한 상담 일정 관리 업무 부담 및 방문자 응대 문제에서 시작된 프로젝트. 데스크에 사람이 없어도 로봇은 방문자를 목적지까지 안내, 방문자는 전화 대신 웹 예약, 관리자는 관제 화면을 통해 일정·로봇 현황 관리 등 상담 예약부터 방문자 안내까지 자동화하도록 고안된 멀티 로봇 안내 시스템.

- **운영 시나리오**: 예약Web(상담 예약) → 방문자 UI(예약 조회 / 현장 접수) → 임무 생성 → 대기 중인 로봇 자동 배정 → 음성 안내와 함께 목적지 주행 → 상담실 도착 및 음성 안내 → 홈 정밀 제어 복귀 → 임무 대기
- **핵심 도전 과제**: ① 상태 관리 기반 멀티 로봇 제어 시스템 ② 위험(유리문) 구간에서도 안정적 자율주행 ③ 위험 요인(사람·로봇)에 대해 안전 정지 ④ 실제 주행 환경에서 위치추정 오차 최소화 (유리 및 복도 구간, 평평하지 않은 바닥) 
