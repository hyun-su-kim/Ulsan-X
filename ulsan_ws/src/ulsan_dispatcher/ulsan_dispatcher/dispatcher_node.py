...
#
# 구독:
#   /limo1/robot_status (domain 5, ulsan_bridge가 domain 6에서 브릿징)
#   /limo2/robot_status (domain 5, ulsan_bridge가 domain 7에서 브릿징)
#     → 상태 변경 시 POST /robots/{id}/status → FastAPI in-memory 업데이트
#
# 발행:
#   /limo1/goal_destination (domain 5 → ulsan_bridge → domain 6)
#   /limo2/goal_destination (domain 5 → ulsan_bridge → domain 7)
#   /limo1/speak_text       (domain 5 → ulsan_bridge → domain 6)
#   /limo2/speak_text       (domain 5 → ulsan_bridge → domain 7)
#
# 폴링 (0.5초 간격):
...
class DispatcherNode(Node):
    def __init__(self):
        super().__init__('ulsan_dispatcher')
        ...
        # ulsan_bridge가 /limo1/robot_status (domain 6→5)를 이미 브릿징하므로
        ...
        self._diag_updater.setHardwareID('ulsan_dispatcher')
        self._diag_updater.add('ulsan_dispatcher', self._diag_check)
        ...
        self.get_logger().info(f'ulsan_dispatcher 시작 — API: {self._api}')
    ...
    def _dispatch(self, mission: dict, robot: str) -> None:
        ...
        # GuideGoal(목적지+tts) 발행 → ulsan_bridge → ulsan_behaviour FSM
