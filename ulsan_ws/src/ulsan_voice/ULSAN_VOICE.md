...
**입력**: 마이크 (상시)
**출력**: `/goal_destination` 토픽 → `ulsan_behaviour`가 받아서 로봇 출발
...
TTS        ← "{목적지}로 안내해드릴게요." 출력
 ↓
/goal_destination 발행 → ulsan_behaviour
```
...
NLU가 목적지 키의 유효성을 검사하기 위해 `ulsan_behaviour` 패키지의 `waypoints.yaml`을 참조한다.
`get_package_share_directory('ulsan_behaviour')`로 경로를 동적으로 가져오므로 경로 하드코딩 없음.
...
- `load_waypoints()`: ulsan_behaviour의 waypoints.yaml 로드
...
# ulsan_behaviour 먼저 빌드 (ulsan_voice가 waypoints.yaml 참조)
colcon build --packages-select ulsan_behaviour
colcon build --packages-select ulsan_voice
...
ros2 run ulsan_voice voice_node
```
...
> **전제**: PulseAudio가 도는 사용자 세션에서 실행(로봇 데스크톱 세션 / `XDG_RUNTIME_DIR` 잡힌 SSH). `ros2 launch ulsan_voice voice_launch.py`로 띄우면 `voice_params.yaml`을 읽는다.
...
source ~/Ulsan-X/activate_voice.sh
source ~/Ulsan-X/ulsan_ws/install/setup.bash
ros2 launch ulsan_voice voice_launch.py
```
...
```bash
# TTS 확인
python3 -c "from ulsan_voice.tts import TTS; TTS().speak('테스트입니다.')"

# NLU 확인
python3 -c "from ulsan_voice.nlu import extract_destination; print(extract_destination('1강의실 가고 싶어요'))"
```
...
6. NLU 실행
   ...
   → ulsan_behaviour waypoints.yaml에 classroom_1 존재 확인

7. TTS 확정 안내 + /goal_destination 발행
   → "1강의실로 안내해드릴게요." 출력
   → ulsan_behaviour IdleState 수신
   → 로봇 1강의실로 출발
```
...
```
ulsan_coordinator  →  /on_duty        →  ulsan_voice  →  /goal_destination  →  ulsan_behaviour
```

`ulsan_coordinator`(노트북)가 어느 로봇이 응대 담당인지 결정하고,
`ulsan_voice`는 담당일 때만 웨이크워드를 처리해 불필요한 GPU 소모와 동시 응대를 방지한다.
