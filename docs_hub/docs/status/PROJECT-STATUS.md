...
| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| ① | **spin failed 원인 규명·해결** | `done (2026-06-11)` | 실기기에서 더 이상 재현되지 않아 해소 확인(원인 미확정 — 홈/staging 좌표 현장 조정 이후 소멸 추정) |
| ② | **도착 TTS 발화-복귀 동기화** | `부분 (2026-06-11)` | **구현 완료**(출발 DEC-048 방식의 `/speak` 발화-후-복귀 적용, 커밋 984450bc). **실기기 검증 남음** |
...
- [ ] ~~**백엔드 확장성 정리 (데모 후)**~~ — **보류·범위 밖** (2026-06-08): 데모 후 추가 개발은 난반사 BT 복구·도킹 실패 복구 2건만 유지(Phase 4 참고). 본 항목은 필요 시 재개.
