...
>_상담 예약 Web 으로 상담을 예약하고, 방문자 UI 를 통해 예약을 확인하면 음성과 함께 멀티 안내 로봇이 목적지까지 안내한다._
...
학원을 방문하는 사람을 관계자가 일일이 안내하지 않아도 되도록 하고, 상담 일정 관리부터 목적지 안내·복귀 후 재임무 수행까지 전 과정을 자동화했다. 로봇 2대가 동시에 운영되며, 서로 마주쳐도 우선순위에 따라 양보하고, 사람이 감지되면 멈췄다가 다시 출발한다.
...
PyQt5 멀티로봇 대시보드. 지도·두 로봇 실시간 위치/상태·실시간 카메라·긴급 수동 제어·예약 내역·이벤트 로그를 한 화면에서 관제한다. 로봇 상태는 `diagnostics` 기반으로 표시해 연결 끊김까지 감지한다.
...
방문자가 휴대폰·PC로 상담을 사전 예약하는 React 웹(`ulsan-web-ui`). 예약 가능 시간대를 실시간 조회해 **마감된 슬롯은 비활성화**하고, 예약 완료 시 배정 상담실을 즉시 안내한다.
...
학원 입구 키오스크/노트북에서 방문자가 접수하는 React 화면(`ulsan-visitor-ui`). **예약 조회**(이름 + 전화 끝 4자리)와 **현장 방문**(예약 없이 방문 / 강의실 안내) 두 흐름을 제공하고, 접수가 끝나면 대기 중인 로봇에 임무가 배정된다. 모든 로봇이 안내 중이면 대기 안내 화면을 띄운다.
...
> _MySQL — `reservations`(예약) 1 : 0..N `missions`(임무), `logs`(이벤트). 예약 확정·현장 접수 시 `mission`을 생성하고, 디스패처가 IDLE 로봇에 배정한다._
...
로봇의 최상위 제어는 **Yasmin 기반 계층형 FSM**으로, 모든 임무 흐름을 5개 상태로 관리한다. 주행 중 정지 요인(사람·관제·트래픽)은 `WAITING`으로 모이고, 모든 주행/도킹 실패는 `FAILED`로 통합되어 관리자 물리 복구를 기다린다.
...
두 로봇이 일정 거리 내로 접근하면 우선순위를 판정(GUIDING > RETURNING, 동순위 시 LIMO 1)해 낮은 쪽을 일시정지시킨다. 동적 costmap 회피가 아닌 **상태 기반 양보**라, 두 로봇의 거리가 근접할 때에도 한 대가 확실히 멈춰 충돌이 구조적으로 발생하지 않는다.

---

### 사람 감지 정지

YOLOv8(COCO 사전학습) + Depth 게이팅으로 주행 경로 위 **근거리 사람을 감지하면 즉시 정지**하고, 사람이 비키면 재개한다. Depth로 약 1.5m 안쪽만 트리거해 원거리 보행자에는 반응하지 않으며, clear-hold 디바운스로 정지/재개 떨림을 제거한다. 감지 결과는 아래 통합 정지 게이트로 전달된다.

---

### 통합 정지 게이트 (motion_hold)

사람 감지 · 관제 일시정지 · 트래픽 양보 3종을 행동 FSM이 OR로 합쳐 단일 신호(`/motion_hold`)로 표준화하고, 커스텀 Nav2 BT 노드 `MotionHoldCondition`이 이를 받아 주행을 정지한다. **Nav2 목표를 취소하지 않고 멈추므로**, 정지 요인이 사라지면 즉시 재개된다.
...
홈 복귀 시 ArUco 마커 기반 **PBVS(Position-Based Visual Servoing)**로 정밀 정차한 뒤 AMCL 위치를 리셋한다. Nav2가 staging까지 대략 이동하고, staging→Home 구간은 마커를 보며 **staged 3단계 제어(조준→직진→정렬)**로 ~1cm 정밀 정차한다.
