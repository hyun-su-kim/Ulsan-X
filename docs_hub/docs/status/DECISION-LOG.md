...
### DEC-016: 유리 구간 AMCL 불안정 해결 방식 + 유리문 운용 정책
- **Context**: 유리 회전 통과 구간에서 LiDAR가 유리를 투과해 스캔 특징점이 없어 AMCL 파티클이 수렴하지 못함. 실기기 테스트(2026-04-28) 결과: 로봇이 유리 구간에서 빙글빙글 돌며 통과 지연. 옆에 장애물이 있을 때는 빠르게 통과 → AMCL 스캔 특징점 부족이 원인으로 확인. 맵 후보정 확장 및 인플레이션 조정으로는 해결 불가.
- **확인된 사실**:
  - LiDAR는 유리를 감지하지 못함 (SLAM 시 종이 부착 이유). 소프트웨어로 보완 불가.
  - 유리문 닫힘 시 경로 생성 불가 → **운용 정책: 유리문은 항상 열린 상태 유지 가정**
  - 초음파 센서 연동(닫힘 감지 + 음성 대기)은 Phase 4로 이연
- **Options (마커 방식 비교)**:
  - A) 레트로리플렉터 마커: LiDAR 기반. SLAM 재수행 필수. 소프트웨어 변경 없음. AMR 업계 표준. AMCL 위치 오차 60.49% 감소 실측(논문 기준).
  - B) ArUco/AprilTag 마커: 카메라 기반(유리 투과 무관). SLAM 재수행 불필요. AMCL 보정 노드 구현 필요(중간 난이도). LIMO에 Orbbec Astra 카메라 기탑재. 종이 출력으로 즉시 테스트 가능. 위치 정확도 1.2cm/자세 1~2°.
- **Decision**: **B — ArUco 마커 채택**
- **Rationale**:
  - SLAM 재수행 없이 즉시 적용 가능 (레트로리플렉터는 SLAM 재수행 필수).
  - 카메라 기반이므로 유리 투과 여부와 무관하게 동작.
  - LIMO에 Orbbec Astra 카메라 기탑재 → 추가 하드웨어 비용 없음.
  - 종이 출력(10cm × 10cm)으로 즉시 테스트 가능.
  - AMCL 보정 노드(`wego_aruco`) 구현 = 센서 퓨전 경험으로 포트폴리오 어필 가능.
  - 면접 어필: "LiDAR 단일 센서의 한계를 카메라 기반 시각 마커로 보완하는 멀티모달 로컬라이제이션을 구현했다."
- **마커 사양**: ArUco DICT_4X4_50, 10cm × 10cm, 종이 인쇄
- **구현 패키지**: `wego_aruco` (신규, ament_python) — 상세 설계는 `docs/ref/ARUCO-LOCALIZER.md`
- **Date**: 2026-04-28 (결정: 2026-04-29)
