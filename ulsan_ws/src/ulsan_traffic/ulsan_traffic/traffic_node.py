...
# ulsan_behaviour가 발행하는 상태값 중 주행 중인 상태
...
class TrafficNode(Node):
    ...
    def __init__(self):
        ...
        self._diag_updater.setHardwareID('ulsan_traffic')
        self._diag_updater.add('ulsan_traffic', self._diag_check)

        self.create_timer(0.2, self._tick)  # 5 Hz
        self.get_logger().info(
            f'ulsan_traffic 시작  pause={self._pause_dist}m  resume={self._resume_dist}m'
        )
