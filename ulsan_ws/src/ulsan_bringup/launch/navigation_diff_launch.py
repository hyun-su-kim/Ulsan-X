...
def generate_launch_description():
    ...
    ulsan_nav_share_dir = get_package_share_directory('ulsan_2d_nav')

    # setting for rviz configuration path
    rviz_file_name = 'navigation.rviz'
    rviz_config_path = os.path.join(ulsan_bringup_share_dir, 'rviz', rviz_file_name)

    # set the parameters
    parameter_file_name = 'diff_navigation_params.yaml'
    parameter_file_path = os.path.join(ulsan_nav_share_dir, 'params', parameter_file_name)

    # set the map yaml file
    map_file_name = 'map.yaml'
    map_file_path = os.path.join(ulsan_nav_share_dir, 'maps', map_file_name)
